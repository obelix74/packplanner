...
class GearBaseTableViewController: UITableViewController, NavigationStyling {
    lazy var settings: SettingsEntity = SettingsManagerCD.SINGLETON.settings
    ...
    @objc private func handleCloudKitChange() {
        print("📡 CloudKit data changed, refreshing gear")
    ...
    func getGearBrain(_ search: String) -> GearBrainCD {
        return GearBrainCD.getFilteredGears(search: search)
    }

    func loadGear(search: String = "") {
        // Clean up duplicates on first load (when no search)
        if search.isEmpty {
            GearBrainCD.cleanupDuplicateGears()
        }
